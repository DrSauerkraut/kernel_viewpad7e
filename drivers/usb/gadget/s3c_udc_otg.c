...
#define USB_DYNAMIC		1
#define USB_HOST_DYNAMIC	0

static struct work_struct		usb_monitor_work;
...
int check_irq_depth(unsigned int irq);
...
static void stop_usb_otg(void)
{
	struct s3c_udc *dev = the_controller;

	if (regulator_is_enabled(dev->udc_vcc_a))
	{	/* ldo8 regulator off */
 ...
 * to any USB host.
 */
static struct delayed_work otg_init_delaywork;
static void otg_delay_work(struct work_struct *work)
{
	udc_enable(the_controller);
...
int s3c_vbus_enable(struct usb_gadget *gadget, int enable)
{
	...
	if (dev->udc_enabled != enable) {
		...
		if (!enable) {
			...
			//udc_enable(dev);
			//add
			schedule_delayed_work(&otg_init_delaywork, 0.4*HZ);
			//add
		}
	} else
...
static int is_usb_storage_plugin = 0;

#if  USB_DYNAMIC	// for USB OTG module decrese power consumption
void usb_work(struct work_struct *work)
{
	struct s3c_udc *dev = the_controller;
	int i, io_level,io_host_level;
	int err = 0;

	err = gpio_request(USB_DETECT_GPIO,"USB_DETECT");
	if(err != 0)
	{
		printk("****gpio_request: USB_DETECT failed****");
		return;
	}
	io_level = __gpio_get_value(USB_DETECT_GPIO);
	gpio_free(USB_DETECT_GPIO);

	err = gpio_request(USB_ID_GPIO,"USB_IDPIN");
	if(err != 0)
	{
		printk("****gpio_request: USB_IDPIN failed****");
		return;
	}
	io_host_level = __gpio_get_value(USB_ID_GPIO);
	gpio_free(USB_ID_GPIO);
		
	printk("****get into usb work function****,and io_level is %d\n", io_level);
	if (io_level == 0)	//(io_level_host==0) // USB out, set USB module to suspend
	{
		//printk("debug: %s, %d, %s>>>>>>>>>\n", __FILE__, __LINE__, __FUNCTION__);	
		if(is_usb_storage_plugin == 1)
		{
			//wake_unlock(&usb_lock);
			is_usb_storage_plugin = 0;
		}
	...
	} else if(io_level == 1)	//(io_level_host==0)		// USB insert, set USB module to resume
	{	
		//printk("debug: %s, %d, %s>>>>>>>>>\n", __FILE__, __LINE__, __FUNCTION__);	
		if(io_host_level == 1 && is_usb_storage_plugin == 0)
		{
			//wake_lock(&usb_lock);
			is_usb_storage_plugin = 1;
		}
#if 0
		if (!regulator_is_enabled(dev->udc_vcc_a))					
		{	
	...
	enable_irq(__gpio_to_irq(USB_DETECT_GPIO));

}



static irqreturn_t s3c_udc_otg_power_irq(int irq, void *handle)  
{
	
	disable_irq_nosync(__gpio_to_irq(USB_DETECT_GPIO));
	schedule_work(&usb_monitor_work);
...
static int s3c_udc_probe(struct platform_device *pdev)
{
...
#endif
	//add
	INIT_DELAYED_WORK(&otg_init_delaywork, otg_delay_work);
	//add
	...
	dev->udc_vcc_d = regulator_get(NULL, "pd_io");
	dev->udc_vcc_a = regulator_get(NULL, "pd_core");
	...
	regulator_enable(dev->udc_vcc_d);	// open otg power
	regulator_enable(dev->udc_vcc_a);		
...
#if 0
#if (USB_DYNAMIC)||(USB_HOST_DYNAMIC)	
...
#if USB_DYNAMIC	
	INIT_WORK(&usb_monitor_work, usb_work);
	s3c_gpio_cfgpin(USB_DETECT_GPIO, S3C_GPIO_SFN(0xf));
	s3c_gpio_setpull(USB_DETECT_GPIO, 0);


	retval = 
		request_irq(__gpio_to_irq(USB_DETECT_GPIO), s3c_udc_otg_power_irq, IRQ_TYPE_EDGE_BOTH, driver_name, dev);

	if (retval != 0) {
		printk(KERN_ERR "%s: can't get irq %i, err %d\n", driver_name,
		      __gpio_to_irq(USB_DETECT_GPIO), retval);
	}
	...
	int io__host_level;
	err = gpio_request(USB_DETECT_GPIO,"USB_DETECT");
	err1 = gpio_request(USB_ID_GPIO,"USB_IDPIN");
	if (!err && !err1)
	{
		io_level = gpio_get_value(USB_DETECT_GPIO);	
		gpio_free(USB_DETECT_GPIO);
		io__host_level = gpio_get_value(USB_ID_GPIO);	
		gpio_free(USB_ID_GPIO);
		if (io_level == 1 && io__host_level == 1 && is_usb_storage_plugin == 0)		
		{
			//wake_lock(&usb_lock);	
			is_usb_storage_plugin = 1;
		}
	...
	s3c_gpio_setpull(USB_ID_GPIO, 2);
		...
		err = gpio_direction_output(USB_CONTROL2_GPIO,  0 );
		if (err)
			goto err_reset;
...
#if (USB_HOST_DYNAMIC)||(USB_DYNAMIC)
	err_reset:
	    return err;
#endif
...
static int __init udc_init(void)
{
	...
	//add by luis
	//wake_lock_init(&usb_lock, 0, "usb_lock");
	//add by luis

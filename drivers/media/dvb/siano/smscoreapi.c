...
void smscore_registry_setmode(char *devpath, int mode)
{
	...
	entry = smscore_find_registry(devpath);
	//add by luis
	printk("%s, %d, devpath = %s\n", __FILE__, __LINE__, devpath);
	//add by luis
	if (entry)
		entry->mode = mode;
	else{
...
int smscore_register_device(struct smsdevice_params_t *params,
	...
	//add by luis
	printk("debug: %s, %d, %s>>>>>>>>>>>>>>>>>>>>\n", __FILE__, __LINE__, __FUNCTION__);
	printk("allocated %d buffers", dev->num_buffers);
	//add by luis
	...
	*coredev = dev;
	//add by luis
	printk("debug: %s, %d, %s>>>>>>>>>>>>>>>>>>>>\n", __FILE__, __LINE__, __FUNCTION__);
	printk("core device 0x%p created, mode %d devpath %s", dev, dev->mode, dev->devpath);
	//add by luis
...
//add by luis
 ...
 /* add firmware download here */
    ...
    sprintf(tmpname, "%s/%s", "/system/lib/firmware", fw_filename);
	printk("debug: %s, %d, %s, %s>>>>>>>>>>>>>>>>>>>>>\n", __FILE__, __LINE__, __FUNCTION__, tmpname);
    fp = filp_open(tmpname, O_RDONLY, 0);

    if (IS_ERR(fp))
    {
            fp = NULL;
            sms_info("Open firmware file failed");
            return -EINVAL;
    }
	

    inode = fp->f_dentry->d_inode;
    fw_buf_size = inode->i_size;
    printk("file_size=%d\n", fw_buf_size);

    fw_buf = kmalloc(ALIGN(fw_buf_size, SMS_ALLOC_ALIGNMENT),
                    GFP_KERNEL | GFP_DMA);
    if (!fw_buf) {
            sms_info("failed to allocate firmware buffer");
            return -ENOMEM;
    ...
    fs = get_fs();
    set_fs(KERNEL_DS);
    fp->f_op->read(fp, fw_buf, fw_buf_size, &(fp->f_pos));
    set_fs(fs);
	...
	if (coredev->device_flags & SMS_DEVICE_FAMILY2) {
		...
		if (!(coredev->modes_supported & (1 << mode))) {
			...
			*/
			...
			//add by luis
			printk("%s, %d, firmware download success", __FILE__, __LINE__);
			//add by luis
			...
			sms_info("mode %d is already supported by running "
					"firmware", mode);
			//add by luis
			printk("%s, %d ,mode %d is already supported by running "
					"firmware", __FILE__, __LINE__, mode);
			//add by luis
		}
		...
		if (rc < 0) {
			...
			//add by luis
			printk("%s, %d, device init failed, rc %d.", rc, __FILE__, __LINE__);
			//add by luis
		}
		...
		if (mode < DEVICE_MODE_DVBT || mode > DEVICE_MODE_DVBT_BDA) {
			sms_err("invalid mode specified %d", mode);
			//add by luis
			printk("%s, %d, invalid mode specified %d", mode, __FILE__, __LINE__);
			//add by luis
			return -EINVAL;
		}

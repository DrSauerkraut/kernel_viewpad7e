...
static struct early_suspend e_s;
static struct platform_device *priv;
...
struct input_dev *s3c_pwr_global = NULL;
void SendPowerbuttonEvent( void )
{
	if (s3c_pwr_global != NULL) 
	{	
		input_report_key(s3c_pwr_global, GPIO_KEY_POWER, 1);
		input_report_key(s3c_pwr_global, GPIO_KEY_POWER, 0);
...
static void gpio_keys_key_report_event_irq(struct work_struct *work)
{
...
static int __devinit gpio_keys_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		setup_timer(&bdata->timer,
			...
			bdata->gk_state = GPIOKEY_STATE_IDLE;
			INIT_WORK(&bdata->work, gpio_keys_key_report_event_irq);
	...
	s3c_pwr_global=input;

	#ifdef CONFIG_HAS_EARLYSUSPEND
//#if 0
	e_s.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	e_s.suspend = gpio_keys_early_suspend;
	e_s.resume = gpio_keys_late_resume;

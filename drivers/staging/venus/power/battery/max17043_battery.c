...
//Johnson add start
#define BATTER_LOW_RESERVED 8
#define BATTERY_HIGH_RESERVED 95
#define BATTERY_FULL_TEMP 99
//Johnson add end
...
//Johnson modify start
int send_battery_full_temp(int i)
	{
		return set_battery_capacity(i);
	}

//battery capacity reserved to low 8% and high 100%
#ifdef CONFIG_BATTERY_RESERVED
static int capacity_reserved(int capacity)
{
	if (capacity <=BATTER_LOW_RESERVED)
	capacity = 0;
	else if (capacity <= BATTERY_HIGH_RESERVED)
	capacity = (capacity-BATTER_LOW_RESERVED)*BATTERY_FULL_TEMP/(BATTERY_HIGH_RESERVED-BATTER_LOW_RESERVED);
	else
  	capacity=send_battery_full_temp(capacity);
	//capacity=100;
   	return capacity;
}
#endif
...
static int max17044_battery_rsoc(struct max17044_device_info *di)
{
	...
	//rsoc = ((rsoc >> 8) & 0xff);
	rsoc = ((rsoc >> 8) & 0xff)/2; //Johnson modify for new battery module
	//printk( "%s capacity is %d\n",__FUNCTION__,rsoc);
//battery capacity reserved 25%
#ifdef CONFIG_BATTERY_RESERVED
	rsoc = capacity_reserved(rsoc);
	//printk( "%s capacity is %d\n",__FUNCTION__,rsoc);
#else
...
/*****************set 64 byte mode ******************************/ 
	char data0[] = {0xA2,0xE0,0xAB,0xC0,0xB4,0x50,0xB6,0xF0,
			  0xB8,0x60,0xBB,0x10,0xBD,0x30,0xBE,0x50};

	char data1[] = {0xBF,0x30,0xC0,0x40,0xC1,0xD0,0xC4,0xE0,
			  0xC7,0x80,0xC9,0x40,0xCB,0x00,0xCC,0xD0};

	char data2[] = {0x00,0x20,0x02,0x00,0x0A,0xE0,0x0F,0x00,
			  0x21,0xC0,0x37,0x00,0x2E,0x20,0x38,0x20};

	char data3[] = {0x2E,0x20,0x28,0x60,0x12,0x00,0x14,0xE0,
			  0x14,0xE0,0x14,0xE0,0x12,0xE0,0x14,0x40};
...
/********************************set 64 byte mode******************************/
	char data0[] ={0xA5,0x90,0xB2,0xE0,0xB9,0xF0,0xBB,0x40,0xBC,0x20,0xBC,0x60,0xBC,0xA0,0xBC,0xE0};

	char data1[] = {0xBD,0x30,0xBE,0x80,0xBF,0x70,0xC0,0xA0,0xC3,0xA0,0xC7,0x00,0xCB,0xA0,0xD0,0x50};

	char data2[] = {0x01,0xF0,0x07,0xF0,0x1E,0xA0,0x27,0xF0,0x7F,0xE0,0x7F,0xE0,0x7D,0xF0,0x63,0xF0};

	char data3[] = {0x2E,0x00,0x3F,0xF0,0x23,0x30,0x1C,0x70,0x19,0x60,0x13,0x90,0x13,0xF0,0x13,0xF0};
...
{	
	...
	max17044_write_rccomp(client, 0x9318);			//4.write rcomp to a  value of 0x930c, as define lower battery thread is 8%
}

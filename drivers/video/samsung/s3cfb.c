...
//show big logo in kernel(800*600)
#include "s3cfb.h"
#if defined(CONFIG_PRODUCT_V7E)
#include "../logo/truecolor_logo_v7e.h"
#elif defined(CONFIG_PRODUCT_KS8)
#include "../logo/truecolor_logo_ks8.h"
#else
#if (PRODUCT_AT)
#include "../logo/truecolor_logo_at&t.h"
#else
#include "../logo/truecolor_logo_rc.h"
#endif

#endif
static unsigned char *logo_rc = truecolor_logo_data;
extern int fb_show_truecolor_logo(struct fb_info *info, unsigned char *logo_rc, int rotate);
...
//show big logo in kernel(800*600)
#if 1
	printk("Show ture color logo\n");
//#if defined(CONFIG_PRODUCT_V7E)
//#else
	fb_show_truecolor_logo(fbdev->fb[pdata->default_win], logo_rc, FB_ROTATE_UR);
//#endif
...
void s3cfb_shutdown(struct platform_device *pdev)                                  
{
...
#ifndef CONFIG_PRODUCT_PDC3_FOG
	...
	pdata->clk_off(pdev, &fbdev->clock);
	//    }                                                                                                                                    
	// printk("***get into s3cfb shutdown********* \n");
	if (pdata->shut_down)
		pdata->shut_down(pdev);
}

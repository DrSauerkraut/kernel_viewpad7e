...
static void smdkc110_rtc_enable_nodev(int en)
{
	void __iomem *base = smdkc110_rtc_base;
	unsigned int tmp;
	...
	if (!en) {
		...
		if ((readw(base+S3C2410_RTCCON) & S3C2410_RTCCON_RTCEN) == 0) {
			// printk( "rtc disabled, re-enabling\n");
...
#if 0
int smdkc110_rtc_gettime_nodev(struct rtc_time *rtc_tm)
{
	unsigned int have_retried = 0;
	void __iomem *base = smdkc110_rtc_base;

	smdkc110_rtc_enable_nodev(1);
 retry_get_time:
	rtc_tm->tm_sec  = readb(base + S3C2410_RTCSEC);
	rtc_tm->tm_min  = readb(base + S3C2410_RTCMIN);
	rtc_tm->tm_hour = readb(base + S3C2410_RTCHOUR);
...
int smdkc110_rtc_settime_nodev(struct rtc_time *tm)
{
	void __iomem *base = smdkc110_rtc_base;
	int year = tm->tm_year;
	int year100;
	...
	/* we get around y2k by simply not supporting it */
	if (year < 0 || year >= 1000) {
		printk( "rtc only supports 0~999 years\n");
		return -EINVAL;
	}
 ...
 */
static int s3c_rtc_valid_tm(struct rtc_time *tm)
{
	int ret = 0;
	...
	/* Is the time between 2010 to 2030? */
	if (tm->tm_year < 110 || tm->tm_year > 130)
		return -EINVAL;
...
static int __devinit smdkc110_rtc_probe(struct platform_device *pdev)
{
	...
	if (s3c_rtc_valid_tm (&tm) != 0)
	{
		/* Set the default time. 2011:9:30:12:0:0 */
		tm.tm_year = 111;
		tm.tm_mon = 8;
		tm.tm_mday = 30;
		tm.tm_hour = 12;

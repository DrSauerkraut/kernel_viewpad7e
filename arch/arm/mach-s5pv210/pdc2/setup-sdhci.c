...
void s5pv210_setup_sdhci2_cfg_gpio(struct platform_device *dev, int width)
{
	...
	case 4:
		...
		#if 0
		gpio = S5PV210_GPH2(3);      // configuate as extern interrupt
		s3c_gpio_cfgpin(gpio, S3C_GPIO_SFN(0xF));
		s3c_gpio_setpull(gpio, 0);

		gpio = readl(S5P_EINT_CON(2));  // configuate as both edge triggered
		gpio &= ~(0xf << 12);
		gpio |= 0x4 << 12;
		writel(gpio, S5P_EINT_CON(2));
		#endif
		break;
	default:
		printk(KERN_ERR "Wrong SD/MMC bus width : %d\n", width);
...
#endif
	card_status = gpio_get_value(S5PV210_GPH2(3));
	//printk(KERN_DEBUG "Universal : Card status %d\n", card_status ? 0 : 1);
	// printk("Card status %d\n", card_status ? 0 : 1);

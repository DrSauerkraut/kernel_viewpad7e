...
/* fsysky: POP_MUTE_SPECIAL
 * We need to mute spk(or hp) when it is on.
 * But we need to check whether spk(or hp) is on,
 * so that we can mute or un-mute it correctly.
 * Note that, on or off is a logic thing, so we 
 * judge on or off when normal set.
 * special_set: if eq 0, normal set;
 */
#define POP_MUTE_SPECIAL
#ifdef POP_MUTE_SPECIAL
static int special_set=0, hp_on=0, spk_on=0;
#endif
	...
	unsigned int value)
{
	...
	BUG_ON(reg > WM8994_MAX_REGISTER);
        ...
        if (!wm8994_volatile(reg)) {
                ...
                case WM8994_SPEAKER_VOLUME_RIGHT:
                        if(special_set) {
                                /* When special setting occur, it will stop normal
                                 * setting do the same thing(same value). This cause
                                 * hp_on & spk_on not to be updated. To aviod this,
                                 * cache should not be set when special setting */
                                break;
                        }
                default:
	...
	if (!wm8994_volatile(reg))
...
#ifdef POP_MUTE_SPECIAL
        /* mute specail to seperate mute operation form other */
        switch (reg) {
        case WM8994_LEFT_OUTPUT_VOLUME:
        case WM8994_RIGHT_OUTPUT_VOLUME:
                if(!special_set) {
                        hp_on = (value & 0x40) ? 1 : 0;
                } else {
                        special_set = 0;
                }
                break;
        ...
        /* fsysky: DAC1(L/R) being disabled and enable again, may
           cause no sound, so we keep DAC1(L/R) always enabled. */
        if(reg == 0x5) value |= 0x303;
...
static int wm8994_aif_mute(struct snd_soc_dai *codec_dai, int mute)
{
	...
	int reg;
        ...
        if(hp_on) {
                if(!mute) {
                        /* special setting will not change cache, 
                         * so when un-mute, the registers may not 
                         * be updated, to confirm updated, we write
                         * the reversed value */
                        special_set = 0;
                        snd_soc_update_bits(codec, WM8994_LEFT_OUTPUT_VOLUME, WM8994_HPOUT1L_MUTE_N_MASK, 0);
                        snd_soc_update_bits(codec, WM8994_RIGHT_OUTPUT_VOLUME, WM8994_HPOUT1R_MUTE_N_MASK, 0);
                        snd_soc_update_bits(codec, WM8994_LEFT_OUTPUT_VOLUME, WM8994_HPOUT1L_MUTE_N_MASK, 0x40);
                        snd_soc_update_bits(codec, WM8994_RIGHT_OUTPUT_VOLUME, WM8994_HPOUT1R_MUTE_N_MASK, 0x40);
                        hp_on = 1;
                }
                special_set = 1;
                snd_soc_update_bits(codec, WM8994_LEFT_OUTPUT_VOLUME, WM8994_HPOUT1L_MUTE_N_MASK, reg);
                special_set = 1;
                snd_soc_update_bits(codec, WM8994_RIGHT_OUTPUT_VOLUME, WM8994_HPOUT1R_MUTE_N_MASK, reg);
                /* may be registers were not set due to that value
                 * to be set may be same with the value of register,
                 * so, should clear */
                special_set = 0;
        }
        if(spk_on) {
                if(!mute) {
                        special_set = 0;
                        snd_soc_update_bits(codec, WM8994_SPEAKER_VOLUME_LEFT, WM8994_SPKOUTL_MUTE_N_MASK, 0);
                        snd_soc_update_bits(codec, WM8994_SPEAKER_VOLUME_RIGHT, WM8994_SPKOUTR_MUTE_N_MASK, 0);
                        snd_soc_update_bits(codec, WM8994_SPEAKER_VOLUME_LEFT, WM8994_SPKOUTL_MUTE_N_MASK, 0x40);
                        snd_soc_update_bits(codec, WM8994_SPEAKER_VOLUME_RIGHT, WM8994_SPKOUTR_MUTE_N_MASK, 0x40);
                        spk_on = 1;
                }
                special_set = 1;
                snd_soc_update_bits(codec, WM8994_SPEAKER_VOLUME_LEFT, WM8994_SPKOUTL_MUTE_N_MASK, reg);
                special_set = 1;
                snd_soc_update_bits(codec, WM8994_SPEAKER_VOLUME_RIGHT, WM8994_SPKOUTR_MUTE_N_MASK, reg);
                special_set = 0;
        }
#endif

...
static bool is_resume=false;//add by bill 2011-11-10
...
static const struct snd_soc_dapm_route audio_conn[] = {
	...
	{"Input Mux", "Mic", "Mic Bias"},//Mic Bias
	{"ADC", NULL, "Input Mux"},
	...
	struct snd_soc_dai *dai)
{
	...
	ssm2602_write(codec, SSM2602_SRATE, 0x22);
	ssm2602_write(codec, SSM2602_IFACE, 0x2);
#ifdef CONFIG_PRODUCT_USING_V7E_COST_DOWN
	
	ssm2602_write(codec,SSM2602_PWR,0x02);//power off mic
	mic_mute = ssm2602_read_reg_cache(codec,SSM2602_APANA) | 0x2;
	ssm2602_write(codec,SSM2602_APANA,mic_mute);//mute mic
#else	
	ssm2602_write(codec,SSM2602_PWR,0);
#endif	
	/*start*************suspend/resume noise issue**********add by bill 2011-11-10*/
	...
	/*end***************suspend/resume noise issue**********add by bill 2011-11-10*/
			       ...
			       struct snd_soc_dai *dai)
{
	...
	/* set active */
	ssm2602_write(codec, SSM2602_ACTIVE, 0);
   	ssm2602_write(codec, SSM2602_APDIGI, 0);
      ssm2602_write(codec, SSM2602_PWR, 0);
/*start**********************************add by bill 2011-10-25*/
if(hp_detect){
		hp_detect=false;		
		int i=gpio_get_value(S5PV210_GPH1(3));
		if(i==1){
			int err = gpio_request(S5PV210_GPC0(1), "GPC0");
          if (err)
                printk(KERN_ERR "#### failed to request GPG1 for RSDn Function\n");    	
...
/*end**************************************add by bill 2011-10-25*/
	...
	int i;
        for(i=0;i<10;i++)
        {
                printk("REG[%d]=0x%x\n",i,ssm2602_read_reg_cache(codec, i));
        }
#endif
...
static int ssm2602_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute)
	{
		...
		//set MUTE
		gpio_request(MUTE, "MUTE");
              if (err) 
	        {
		            printk(KERN_ERR "failed to request MUTE for MUTE.\n");
		            return err;
	         }
		gpio_direction_output(MUTE, 0);
	       gpio_free(MUTE);

               //set RSDn 
              gpio_request(RSDn, "RSDn");
              if (err) 
	        {
...
#endif
#define SSM2602_RATES SNDRV_PCM_RATE_44100
#define SSM2602_FORMATS SNDRV_PCM_FMTBIT_S16_LE
...
static int ssm2602_resume(struct platform_device *pdev)
{
	...
	int i,reg;
	u8 data[2];
	is_resume=true;//add by bill 2011-11-10
	u16 *cache = codec->reg_cache;
	...
	ssm2602_write(codec, SSM2602_APANA, APANA_SELECT_DAC |
			APANA_ENABLE_MIC_BOOST | APANA_ENABLE_MIC_BOOST2);
	ssm2602_write(codec, SSM2602_PWR, 0xFF);
...
static int ssm2602_init(struct snd_soc_device *socdev)
{
	...
	ssm2602_write(codec, SSM2602_APANA, APANA_SELECT_DAC |
			APANA_ENABLE_MIC_BOOST | APANA_ENABLE_MIC_BOOST2);
	ssm2602_write(codec, SSM2602_PWR, 0xFF); /*for save power after power on*/
